var shoot1 = {   
    ...
    create: function (){
        ...
        game.time.events.add(Phaser.Timer.SECOND * 60, this.finishAnim, this);
...
this.starEmitter.minParticleScale = 0.2;
	...
	this.starEmitter.maxRotation = 0;
        this.starEmitter.alpha = 0.5;
        
         this.starEmitter.forEach(function(singleParticle) {singleParticle.animations.add('particleAnim',[0,1,2,3],4,true);
                                                            singleParticle.tint = Math.random() * 0xffffff;
...
this.starEmitter.start(false, 1600, 5, 0);
    ...
    generateWave: function(){
        ...
        enemy.body.velocity.x = game.rnd.between(-60,-120);
                                        ...
                                        this.tweenran = game.rnd.between (1,12);
                
                if(this.tweenran==1){
        this.enemyTween = game.add.tween(enemy).to({y: enemy.y + 400}, 800, Phaser.Easing.Sinusoidal.InOut).yoyo(true) .loop() .start();
                    }
                               if(this.tweenran==2){
        this.enemyTween = game.add.tween(enemy).to({y: enemy.y + 250}, 600, Phaser.Easing.Sinusoidal.InOut).yoyo(true) .loop() .start();
                    }
                               if(this.tweenran==3){
        this.enemyTween = game.add.tween(enemy).to({y: enemy.y + 150}, 300, Phaser.Easing.Sinusoidal.InOut).yoyo(true) .loop() .start();
                   enemy.tint = Math.random() * 0xFF0000;
                    }
